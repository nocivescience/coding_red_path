...
class FourierCirclesScene(ZoomedScene):
    ...
    def get_vector_sum_path(self, vectors, color=YELLOW):
        ...
        path = ParametricFunction(
            lambda t: center + functools.reduce(op.add, [
                complex_to_R3(
    ...
    def get_y_component_wave(self,
        ...
        wave = ParametricFunction(
            ...
            t_range=np.array([path.t_min,path.t_max]),
            color=color,
        )
        wave_copies = VGroup(*[
            wave.copy()
...
class AbstractFourierOfTexSymbol(FourierCirclesScene):
    ...
    def construct(self):
        ...
        if self.CONFIG['include_zoom_camera']:
            self.zoom_config()
        if self.CONFIG['scale_zoom_camera_to_full_screen']:
        ...
        if self.CONFIG['n_cycles'] != None:
            if not self.CONFIG['scale_zoom_camera_to_full_screen']:
                for n in range(self.CONFIG['n_cycles']):
                  self.run_one_cycle()
            else:
        ...
        elif self.CONFIG['run_time'] != None:
            self.wait(self.CONFIG['run_time'])
    ...
    def set_decreasing_stroke_widths(self, circles):
        mcsw = self.CONFIG['max_circle_stroke_width']
